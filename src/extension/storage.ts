...
export interface SavedBook {
   ...
   *
   * Stored ONLY when the post held exactly one picture, so this URL is the picture this
   * book was actually read from rather than photograph one of four. Several BOOKS out of
   * that one picture all keep it: none of their covers can be displaced by it, because
   * `coverSources` puts `book.coverUrl` first. See `shotFor`.
   *
