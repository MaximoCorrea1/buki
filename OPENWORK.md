...
| **What a stored record must look like before it is trusted** | **`storedShelf.ts`** (the shelf) and **`storedLog.ts`** (the recognition log). `chrome.storage.local` is user-editable and outlives four schema versions; both DROP a bad row rather than throwing, because one damaged record must not become *"you have no books"* | `Array.isArray` plus a cast, which is what they replaced |
| **Which answer goes with which ask, between contexts** | **`messages.ts`** — `BackgroundReplies` / `ContentReplies`, keyed by the union's own `type`, plus `MESSAGE_CONTRACT_COMPLETE`. A variant with no reply is a COMPILE error | `sendMessage`, which defaults its response to `any` |
| **What counts as the post a book came from** | **`permalink.ts`**. https, host via `feedHost.ts`'s label-boundary rule, `status` as a PATH SEGMENT | a `href*=` substring selector, which stored `javascript:` URLs on the shelf |
| **Whether a card needs repainting** | **`cardSignature.ts`**. Deliberately NOT in the DOM: the string carries every book title and pile, and `[data-sig^="a"]` reads it with no script at all | `el.dataset`, in x.com's own light DOM |
| **How long an edge function waits, and on what** | **`upstreamTimeout.ts`**. The ceilings are chosen RELATIVE to the client's and asserted against those constants — above them, a server bound can never fire | a copied number, or `request.signal`, which is abort propagation and not a timeout |
| **What may cross back from a provider** | **`upstreamRelay.ts`**. Scrubbed and bounded; an oversized SUCCESS is REFUSED rather than truncated, because a truncated success becomes *"no books found"* | relaying `await upstream.text()` verbatim |
...
- [x] **51. THE SERVER'S REMAINING CONTRACT AND EDGE GAPS.** *(review §4 and §5)* **ALL NINE CLOSED 2026-08-27/28.** Every one mutation-tested twice; **81 mutations across the item, 81 caught** (`ls tools/mutations/item-51-*.json`, counted 2026-08-28, not remembered), and five of them found real holes in tests written minutes earlier.
      ...
      - ~~**AC-10 · Polar's response is cast, never validated.**~~ **DONE 2026-08-28.**
        ...
        `'granted'`, which was a 403 to a paying subscriber. **The status code is the whole
        finding:** 403 is not in `worthRetrying`, so `license.ts` DESTROYS the session; 502 is,
        so the customer keeps it and rides the grace window. A valid shape carrying a bad
        status still 403s — *"Polar says revoked"* is an answer about the customer, *"Polar
        sent something unreadable"* is our upstream failing its contract, and collapsing them
        is what made an outage look like a cancellation. ⚠ **THE FILE ALREADY KNEW:**
        `PolarValidation`'s docblock said *"read one as the other and `status` is `undefined`,
        so every renewal 403s… and looks exactly like a revoked subscription"* — written as a
